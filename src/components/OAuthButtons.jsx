...
const OAuthButtonsFunctional = ({ onSuccess, theme }) => {
...
export const OAuthButtons = ({ onSuccess, theme }) => {
  if (!isGoogleAuthEnabled) {
    const isDark = theme?.darkMode || theme === 'dark';
    const buttonClass = isDark ? 'bg-white text-gray-900' : 'bg-gray-900 text-white';
    return (
      <div className="space-y-3">
        <button disabled className={`w-full flex items-center justify-center gap-3 py-3 px-6 rounded-lg font-semibold ${buttonClass} opacity-40 cursor-not-allowed shadow-lg`}>
          <GoogleIcon />
          <span>Sign in with Google</span>
        </button>
      </div>
  ...
  return <OAuthButtonsFunctional onSuccess={onSuccess} theme={theme} />;
};
