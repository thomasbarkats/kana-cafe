  ...
  return (
    <div className={`min-h-screen ${theme.bg} p-4 flex items-center -mb-8`}>
      <div className="w-full max-w-4xl mx-auto">
        <div className={`${theme.cardBg} backdrop-blur-sm rounded-3xl shadow-2xl p-4 lg:p-8 relative`}>

          {/* Header with title and selector */}
          <div className="flex flex-wrap gap-3 justify-between items-start mb-6 lg:mb-8">
          ...
          {!stoppedEarly && (
            <div className="text-center mb-8">
              ...
              <h2 className={`text-2xl lg:text-3xl font-bold ${theme.text} mb-2`}>{t('summary.congratulations')}</h2>
          ...
          <div className={`grid ${isVocabularyMode ? 'grid-cols-1' : 'grid-cols-2'} md:grid-cols-2 lg:grid-cols-3 gap-3 lg:gap-4 mb-6 lg:mb-8`}>
            {sortedStats.map((item) => {
              ...
              return (
                <div
                  key={item.key}
                  className={`rounded-lg p-3 lg:p-4 transition-colors duration-200 ${item.infoText ? 'cursor-help' : ''} ${!isPracticed ? 'opacity-50' : ''}`}
                  style={bgStyle}
                  title={item.infoText || ''}
                >
